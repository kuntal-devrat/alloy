...
#[test] fn chunked_free_reuse() { let mut c=ChunkedArena::new(1024); let _p=c.alloc_region(32,2) as usize; c.sweep_free_list(|_| false, |_,_|{}); assert!(c.free_list_len()>0); let q=c.alloc_free(32,2); assert!(q.is_some()); }
...
#[test] fn heap_guard() { let mut h=ArenaHeap::new(512); let _g=HeapGuard::set(&mut h as *mut _); assert!(!alloy_core::heap::current_heap().is_null()); }
...
#[test] fn shm_f64() { let m=SidecarMemory::new(4096); let o=m.allocate_float64_array(&[3.5]).unwrap(); assert!((m.get_float64_slice(o,1).unwrap()[0]-3.5).abs()<1e-9); }
...
#[test] fn regex_search_simple() { let p=compile_from_str("ab+", "").unwrap(); let chars="abbb".chars().collect::<Vec<_>>(); let m=alloy_core::regex::search(&p,&chars,0).unwrap().unwrap(); assert_eq!(m.start,0); }
#[test] fn regex_case_insensitive() { let p=compile_from_str("abc","i").unwrap(); let chars="ABC".chars().collect::<Vec<_>>(); assert!(alloy_core::regex::search(&p,&chars,0).unwrap().is_some()); }
#[test] fn regex_multiline() { let p=compile_from_str("^b","m").unwrap(); let chars="a\nb".chars().collect::<Vec<_>>(); assert!(alloy_core::regex::search(&p,&chars,0).unwrap().is_some()); }
#[test] fn regex_scan_all() { let p=compile_from_str("a","g").unwrap(); let chars="aba".chars().collect::<Vec<_>>(); assert_eq!(alloy_core::regex::scan_all(&p,&chars).unwrap().len(),2); }
#[test] fn regex_unicode_flag() { assert!(compile_from_str("a","u").is_ok()); }

// Value
#[test] fn value_numbers() { let v=Value::number(3.5); assert!(v.is_number()); }
