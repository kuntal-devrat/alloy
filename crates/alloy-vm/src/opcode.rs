...
pub enum Opcode {
    ...
    /// op + u8 kind.
    SetAccessor = 138,
    /// Yield value from generator. Layout: op (single byte).
    Yield = 139,
    /// Create and return generator object. Layout: op (single byte).
    CreateGenerator = 140,
    /// 3-address register ALU: locals[dst] = locals[src1] op locals[src2]
    BinLocalLocalLocalArith = 141,
    /// 3-address register ALU with immediate: locals[dst] = locals[src] op imm32
    BinLocalLocalLocalInt = 142,
    /// 2-address register copy: locals[dst] = locals[src]
    StoreLocalLocal = 143,
    /// 2-address register compare and jump: if !(locals[src1] cmp locals[src2]) jump target
    CmpLocalLocalJumpIfFalse = 144,
}
...
static OPCODE_TABLE: [Option<Opcode>; 256] = {
    ...
    while i < 256 {
        table[i] = match i as u8 {
            ...
            1 => Some(Opcode::LoadInt),
            2 => Some(Opcode::LoadTrue),
            3 => Some(Opcode::LoadFalse),
            4 => Some(Opcode::LoadNull),
            5 => Some(Opcode::LoadUndefined),
            6 => Some(Opcode::LoadLocal),
            7 => Some(Opcode::StoreLocal),
            8 => Some(Opcode::LoadGlobal),
            9 => Some(Opcode::StoreGlobal),
            10 => Some(Opcode::Add),
            11 => Some(Opcode::Subtract),
            12 => Some(Opcode::Multiply),
            13 => Some(Opcode::Divide),
            14 => Some(Opcode::Modulo),
            15 => Some(Opcode::Negate),
            16 => Some(Opcode::Equal),
            17 => Some(Opcode::NotEqual),
            18 => Some(Opcode::StrictEqual),
            19 => Some(Opcode::Less),
            20 => Some(Opcode::Greater),
            21 => Some(Opcode::LessEqual),
            22 => Some(Opcode::GreaterEqual),
            23 => Some(Opcode::And),
            24 => Some(Opcode::Or),
            25 => Some(Opcode::Not),
            26 => Some(Opcode::Jump),
            27 => Some(Opcode::JumpIfFalse),
            28 => Some(Opcode::JumpIfTrue),
            29 => Some(Opcode::Call),
            30 => Some(Opcode::Return),
            31 => Some(Opcode::MakeArray),
            32 => Some(Opcode::MakeObject),
            33 => Some(Opcode::GetProperty),
            34 => Some(Opcode::SetProperty),
            35 => Some(Opcode::Pop),
            36 => Some(Opcode::Dup),
            37 => Some(Opcode::Halt),
            38 => Some(Opcode::Nop),
            39 => Some(Opcode::TypeOf),
            40 => Some(Opcode::Print),
            41 => Some(Opcode::AllocShared),
            42 => Some(Opcode::ReadShared),
            43 => Some(Opcode::WriteShared),
            44 => Some(Opcode::Send),
            45 => Some(Opcode::Receive),
            46 => Some(Opcode::Spawn),
            47 => Some(Opcode::CaptureLocal),
            48 => Some(Opcode::CaptureUpvalue),
            49 => Some(Opcode::NewClosure),
            50 => Some(Opcode::LoadUpvalue),
            51 => Some(Opcode::StoreUpvalue),
            52 => Some(Opcode::LoadCell),
            53 => Some(Opcode::StoreCell),
            54 => Some(Opcode::LoadSelf),
            55 => Some(Opcode::GetIndex),
            56 => Some(Opcode::SetIndex),
            57 => Some(Opcode::GetKeys),
            58 => Some(Opcode::NewPromise),
            59 => Some(Opcode::Await),
            60 => Some(Opcode::Throw),
            61 => Some(Opcode::TryStart),
            62 => Some(Opcode::TryEnd),
            63 => Some(Opcode::CallSpread),
            64 => Some(Opcode::MakeArraySpread),
            65 => Some(Opcode::MakeRestArray),
            66 => Some(Opcode::ArraySlice),
            67 => Some(Opcode::CmpLocalInt),
            68 => Some(Opcode::BinLocalInt),
            69 => Some(Opcode::BinLocalLocal),
            70 => Some(Opcode::CmpLocalLocal),
            71 => Some(Opcode::ArithStoreLocal),
            72 => Some(Opcode::IncLocal),
            73 => Some(Opcode::ArithStoreUpvalue),
            74 => Some(Opcode::CompoundPropConst),
            75 => Some(Opcode::PeekProperty),
            76 => Some(Opcode::ArithWriteProp),
            77 => Some(Opcode::CompoundIndexConst),
            78 => Some(Opcode::PeekIndex),
            79 => Some(Opcode::ArithWriteIndex),
            80 => Some(Opcode::IncPropConst),
            81 => Some(Opcode::IncIndexConst),
            82 => Some(Opcode::JumpIfFalsePop),
            83 => Some(Opcode::CallKeep0),
            84 => Some(Opcode::CallSpreadKeep0),
            85 => Some(Opcode::JumpIfTruePop),
            86 => Some(Opcode::BitAnd),
            87 => Some(Opcode::BitOr),
            88 => Some(Opcode::BitXor),
            89 => Some(Opcode::BitNot),
            90 => Some(Opcode::StrictNotEqual),
            91 => Some(Opcode::Shl),
            92 => Some(Opcode::Shr),
            93 => Some(Opcode::UShr),
            94 => Some(Opcode::Pow),
            95 => Some(Opcode::DeleteProp),
            96 => Some(Opcode::DeleteIndex),
            97 => Some(Opcode::AppendStringConst),
            98 => Some(Opcode::AppendStringLocal),
            99 => Some(Opcode::AppendStringPop),
            100 => Some(Opcode::LoadPython),
            101 => Some(Opcode::LoadLocalGetPropConst),
            102 => Some(Opcode::LoadLocalLocalGetIndex),
            103 => Some(Opcode::BinIntLocal),
            104 => Some(Opcode::BinLocalLocalInt),
            105 => Some(Opcode::CmpAndLocalLocal),
            106 => Some(Opcode::CmpAndLocalInt),
            107 => Some(Opcode::CmpAndIntLocal),
            108 => Some(Opcode::CmpAndIntInt),
            109 => Some(Opcode::ArithChain),
            110 => Some(Opcode::Arith2StoreLocalConst),
            111 => Some(Opcode::Arith3StoreLocalConstConst),
            112 => Some(Opcode::Arith3StoreConstLocalConst),
            113 => Some(Opcode::TypeOfGlobal),
            114 => Some(Opcode::GetPropertyCell),
            115 => Some(Opcode::LoadThis),
            116 => Some(Opcode::New),
            117 => Some(Opcode::CallMethod),
            118 => Some(Opcode::CallMethodKeep0),
            119 => Some(Opcode::CallMethodSpread),
            120 => Some(Opcode::CallMethodSpreadKeep0),
            121 => Some(Opcode::GetProto),
            122 => Some(Opcode::SetProto),
            123 => Some(Opcode::InstanceOf),
            124 => Some(Opcode::CmpLocalIntJumpIfFalsePop),
            125 => Some(Opcode::CmpLocalLocalJumpIfFalsePop),
            126 => Some(Opcode::LoadIndexCmpLocalJumpIfFalsePop),
            127 => Some(Opcode::ArithLocalIntCmpJumpIfFalsePop),
            128 => Some(Opcode::SetIndexLocalLocal),
            129 => Some(Opcode::SetIndexLocalGetLocal),
            130 => Some(Opcode::SetIndexLocalPlusIntLocalGetLocal),
            131 => Some(Opcode::JumpIfNullish),
            132 => Some(Opcode::ToIterable),
            133 => Some(Opcode::LoadArguments),
            134 => Some(Opcode::In),
            135 => Some(Opcode::WrapCell),
            136 => Some(Opcode::MakeRegex),
            137 => Some(Opcode::NewSpread),
            138 => Some(Opcode::SetAccessor),
            139 => Some(Opcode::Yield),
            140 => Some(Opcode::CreateGenerator),
            141 => Some(Opcode::BinLocalLocalLocalArith),
            142 => Some(Opcode::BinLocalLocalLocalInt),
            143 => Some(Opcode::StoreLocalLocal),
            144 => Some(Opcode::CmpLocalLocalJumpIfFalse),
            _ => None,
        };
        i += 1;
    }
...
impl Opcode {
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            Self::Yield => "YIELD",
            Self::CreateGenerator => "CREATE_GENERATOR",
            Self::BinLocalLocalLocalArith => "BIN_LOCAL_LOCAL_LOCAL_ARITH",
            Self::BinLocalLocalLocalInt => "BIN_LOCAL_LOCAL_LOCAL_INT",
            Self::StoreLocalLocal => "STORE_LOCAL_LOCAL",
            Self::CmpLocalLocalJumpIfFalse => "CMP_LOCAL_LOCAL_JMP_IF_FALSE",
        }
    }
}
