...
pub(crate) fn parse_url(href: &str, base: Option<&str>) -> Option<(String, String, String, String, Vec<(String, String)>, String, String)> {
    ...
    Value::native(Arc::new(|args, vm| {
        ...
        match fetch_sync_inner(&url, &method, &headers, &body, timeout_ms) {
            Ok((status, status_text, rheaders, rbody)) => {
                ...
                let text_fn = Value::native(Arc::new(move |_args, _vm| {
                    Value::string(text_body.clone())
                }));
                m.insert("text".to_string(), text_fn);

                let json_body = body_str;
                let json_fn = Value::native(Arc::new(move |_args, vm| {
                    match super::json::parse_json_str(&json_body) {
                        Ok(val) => val,
                        Err(e) => {
                            vm.throw_exception(Value::string(format!("SyntaxError: Unexpected token in JSON: {}", e)));
                            Value::undefined()
                        }
                    }
                }));
