use super::builtins::http_server::{bind_server, request_complete, serve_loop};
use super::*;
...
fn generational_reclaim_between_runs() {
    ...
    assert_eq!(
        young_after_r1, 0,
        "young generation not reclaimed after run 1"
    );
    ...
    assert_eq!(
        out,
        "run1 persisted-string 42\npersisted-string 42 queued-msg"
    );
    assert_eq!(
        vm.heap_used_young(),
        0,
        "young generation not reclaimed after run 2"
    );
}
...
fn constant_interning_and_program_heap_compaction() {
    ...
    assert_eq!(
        count("repeated-literal"),
        1,
        "string literals must be interned"
    );
    assert_eq!(
        count("longPropertyName"),
        1,
        "property names must be interned"
    );
    assert_eq!(
        count("length"),
        1,
        "builtin property names must be interned"
    );
    ...
    assert_eq!(
        ...
        0,
        "program heap young not reset after compaction"
    );
    ...
    for c in &program.constants {
        if let Some(s) = c.as_str() {
            ...
            assert!(
                program.heap.addr_in_old(b.bytes_ptr() as usize),
                "string bytes not in old"
            );
            boxed += 1;
        }
    }
...
fn side_table_young_density_zero_padding() {
    ...
    assert!(
        regions > 4000,
        "expected a few thousand regions, got {}",
        regions
    );
}
...
fn size_class_bins_lifo_reuse_pins_churned_slot() {
    ...
    assert!(
        vm.heap.free_list_len() <= 2,
        "dead space fragmented: {} regions",
        vm.heap.free_list_len()
    );
}
...
fn rope_concat_linear_and_deep_flatten_exact() {
    ...
    assert!(
        b.is_cons(),
        "loop-built string must stay a rope while building"
    );
...
fn string_accumulator_fusion_emitted_and_correct() {
    ...
    assert!(
        has(Opcode::AppendStringConst as u8),
        "AppendStringConst not emitted"
    );
    assert!(
        has(Opcode::AppendStringLocal as u8),
        "AppendStringLocal not emitted"
    );
    assert!(
        has(Opcode::AppendStringPop as u8),
        "AppendStringPop not emitted"
    );
...
fn peephole_cmp_chains_emitted_and_correct() {
    ...
    let has = |op: u8| program.bytecode.contains(&op);
    assert!(
        has(Opcode::CmpAndLocalLocal as u8),
        "CmpAndLocalLocal not emitted"
    );
    assert!(
        has(Opcode::CmpAndLocalInt as u8),
        "CmpAndLocalInt not emitted"
    );
    assert!(
        has(Opcode::CmpAndIntLocal as u8),
        "CmpAndIntLocal not emitted"
    );
    ...
    assert_eq!(
        out,
        "true true false true false true true true false 11 3 7 4"
    );
}
...
fn register_superinstructions_emitted_and_correct() {
    ...
    assert!(
        has(Opcode::StoreLocalLocal as u8),
        "StoreLocalLocal not emitted"
    );
    assert!(
        has(Opcode::BinLocalLocalLocalArith as u8),
        "BinLocalLocalLocalArith not emitted"
    );
    assert!(
        has(Opcode::BinLocalLocalLocalInt as u8),
        "BinLocalLocalLocalInt not emitted"
    );
    let (_vm, sink) = run_src(src);
    let out = sink.lock().unwrap().join(" ");
...
fn jit_compiles_and_executes_tight_loop() {
    ...
    if let Some(jit) = vm.jit.as_ref() {
        if !jit.is_disabled() {
            assert!(
                jit.cache_len() > 0,
                "Loop should have been compiled by Cranelift JIT"
            );
        }
    }
}
...
fn jit_disabled_flag_falls_back_to_interpreter() {
    ...
    if let Some(jit) = vm.jit.as_ref() {
        ...
        assert_eq!(
            jit.cache_len(),
            0,
            "No loops should be compiled when disabled"
        );
    }
}
...
fn spawn_runs_on_worker_thread_and_settles_promise() {
    ...
    assert_eq!(
        out,
        "sync:499500 closure:30 obj:[object Object] rejected:boom async:42"
    );
}
...
fn require_works_inside_async_server_handler() {
    let dir = std::env::temp_dir().join(format!("alloy_handler_require_{}", std::process::id()));
...
fn require_works_inside_spawn_worker() {
    let dir = std::env::temp_dir().join(format!("alloy_worker_require_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(
        dir.join("math.ajs"),
...
fn reload_propagates_across_threads() {
    let dir = std::env::temp_dir().join(format!("alloy_reload_xthread_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn reload_from_worker_invalidates_worker_cache() {
    let dir = std::env::temp_dir().join(format!("alloy_reload_worker_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn concurrent_require_same_module_no_race() {
    let dir = std::env::temp_dir().join(format!("alloy_race_require_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(
        dir.join("shared.ajs"),
        "export let n = 0;\n\
...
fn live_worker_sees_reload_after_channel_wake() {
    let dir = std::env::temp_dir().join(format!("alloy_reload_wake_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn array_join_push_and_date_now_match_node() {
    ...
    assert_eq!(
        out,
        "1 3 4 1,2,3,four 1-2-3-four [1,2,3,four] [] ||5 1,2+3 true"
    );
}
...
fn array_string_prototype_roundout_matches_node() {
    ...
    assert_eq!(
        out,
        "5 1 5 8,2 1,2,3,4,5 1 true -1 2-4-6 6 e   el el a|b|c a-b-c HELLO 1"
    );
}
...
fn parser_rejects_silent_misparses() {
    ...
    ok("const sq = x => x * x\nprint(sq(5))", "25");
    ok(
        "print(0x10.toString(16), 0b101.toString(2), 0o17.toString(8))",
        "10 101 17",
    );
    ok(
        "print(.5, .5e2, 0..toString(2), 1..toString()) ",
        "0.5 50 0 1",
    );
    ...
    ok(
        "class A {} let a = new A(); print(a instanceof A, 1 instanceof Number, null instanceof A)",
        "true false false",
    );
}
...
fn smi_feedback_lanes_match_node() {
    ...
    assert_eq!(
        out,
        "10 30 75025 742500 1998.046875 1 NaN -0 true false true 15"
    );
}
...
fn string_builder_aliases_stay_valid_and_promote() {
    ...
    assert!(
        b.is_builder(),
        "loop-built string should be a growable builder"
    );
    assert!(
...
fn rope_promoted_then_flattened_lives_in_old_gen() {
    ...
    let check =
        Compiler::compile_source_with_mode("print(big.length, big[0], big[3999]);", true, false)
            .unwrap();
    vm.set_program(check);
    vm.run();
    let out = sink.lock().unwrap().join("\n");
...
fn python_embed_roundtrip() {
    if !embed_mode() {
        eprintln!(
            "skip: run the suite with ALLOY_PYTHON_EMBED=1 to exercise the in-process interpreter"
        );
        return;
    }
    let dir = std::env::temp_dir();
...
fn sweep_segments_native_reclaims_orphans_immediately() {
    ...
    let out = sink.lock().unwrap().join(" | ");
    assert!(
        out.contains("swept 1 7"),
        "sweepSegments must report the orphan, got: {out}"
    );
    assert!(
        out.contains("keys true true"),
        "result object malformed, got: {out}"
    );
...
fn server_same_file_calls_run_in_parallel() {
    ...
    let warm: Vec<_> = (0..2)
        .map(|i| std::thread::spawn(move || http_client(port, &format!("/w{}", i))))
        .collect();
    for t in warm {
    ...
    let clients: Vec<_> = (0..2)
        .map(|i| std::thread::spawn(move || http_client(port, &format!("/{}", i))))
        .collect();
    let mut ok = true;
    for c in clients {
        match c.join() {
...
fn server_handler_error_returns_500() {
    ...
    assert!(
        resp.contains("kaboom from python"),
        "missing reason in: {}",
        resp
    );
    assert!(
        !resp.contains("never-reached"),
        "handler continued after rejection"
    );
    stop.store(true, std::sync::atomic::Ordering::Relaxed);
    server.join().expect("serve thread exited");
}
...
fn express_framework_live_routes() {
    let lib = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../lib/express.ajs");
    if !lib.exists() {
    ...
    let get =
        |p: &str| http_raw(port, &format!("GET {} HTTP/1.1\r\nHost: t\r\n\r\n", p)).expect("req");
    let post = |p: &str, b: &str| {
        http_raw(port, &format!("POST {} HTTP/1.1\r\nHost: t\r\nContent-Type: application/json\r\nContent-Length: {}\r\n\r\n{}", p, b.len(), b)).expect("req")
    };
    let r = get("/api/items/7");
    assert!(
        r.contains("200 OK") && r.contains("\"id\": \"7\""),
        "param: {}",
        r
    );
    let r = post("/api/items", r#"{"name":"x"}"#);
    assert!(
        r.contains("201 Created") && r.contains("made"),
        "create: {}",
        r
    );
...
fn web_crypto_vectors() {
    ...
    assert_eq!(
        web_out(r#"print(crypto.sha256("abc"))"#),
        "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"
    );
    assert_eq!(
        web_out(r#"print(crypto.sha256(""))"#),
        "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
    );
    assert_eq!(
        web_out(r#"print(crypto.hmacSha256("Jefe", "what do ya want for nothing?"))"#),
    ...
    assert_eq!(
        web_out(r#"print(crypto.timingSafeEqual("abc", "abc"))"#),
        "true"
    );
    assert_eq!(
        web_out(r#"print(crypto.timingSafeEqual("abc", "abd"))"#),
        "false"
    );
    assert_eq!(
        web_out(r#"print(crypto.timingSafeEqual("abc", "abcd"))"#),
        "false"
    );
    ...
    assert_eq!(
        web_out(
            r#"const s = crypto.hmacBase64Url("k", "a.b"); print(s === crypto.hmacBase64Url("k", "a.b"))"#
        ),
        "true"
    );
}
...
fn web_uri_codec() {
    assert_eq!(
        web_out(r#"print(encodeURIComponent("a b+c"))"#),
        "a%20b%2Bc"
    );
    assert_eq!(
        web_out(r#"print(encodeURIComponent("~ok-_.!*'()"))"#),
        "~ok-_.!*'()"
    );
    assert_eq!(web_out(r#"print(decodeURIComponent("a%20b"))"#), "a b");
    assert_eq!(
        web_out(r#"print(encodeURI("http://x/?a=b&c=d#f"))"#),
        "http://x/?a=b&c=d#f"
    );
    ...
    let (mut vm, _) = run_src(
        r#"try { decodeURIComponent("%zz"); print("no-throw"); } catch (e) { print("threw"); }"#,
    );
    assert!(vm.take_error().is_none());
    let (_, sink) = run_src(
        r#"try { decodeURIComponent("%zz"); print("no-throw"); } catch (e) { print("threw"); }"#,
...
fn web_url_parse() {
    assert_eq!(
        web_out(
            r#"const u = URL.parse("https://ex.com:8080/p?q=1#h"); print(u.protocol, u.hostname, u.port, u.path, u.hash)"#
        ),
    ...
    assert_eq!(
        web_out(
            r#"const u = URL.parse("http://h/a?x=1&y=two+words"); print(u.query.x, u.query.y)"#
        ),
        "1 two words"
    );
    assert_eq!(
        web_out(
            r#"const u = URL.parse("/rel", "http://base.com/root"); print(u.hostname, u.path)"#
        ),
        "base.com /rel"
    );
    assert_eq!(
...
fn web_server_fields_and_response_controls() {
    let (port, stop, server) = serve_once(
        r#"
            async function handle(req, res) {
                if (req.path === "/st") {
                    res.status(201);
...
fn web_fetch_sync_roundtrip_and_https_refusal() {
    ...
    let out = web_out(
        r#"try { fetchSync("ftp://example.com/"); print("no-throw"); } catch (e) { print("threw"); }"#,
    );
    assert_eq!(out, "threw");
