...
fn typeof_support() {
    ...
    assert_eq!(run_lines("const f = function() {}\nprint(typeof f)"), "function");
}
...
fn break_outside_loop_errors() {
    assert!(matches!(compile_err("break"), CompileError::BreakOutsideLoop));
    assert!(matches!(compile_err("continue"), CompileError::BreakOutsideLoop));
...
fn template_literals() {
    assert_eq!(run_lines("let name = \"World\"\nprint(`Hello ${name}!`)"), "Hello World!");
    ...
    assert_eq!(run_lines("print(`f=${(function(a, b) { return a + b })(2, 3)}`)"), "f=5");
    // Escapes.
...
fn arrow_functions() {
    ...
    assert_eq!(run_lines("const add = (a, b) => a + b\nprint(add(2, 3))"), "5");
    assert_eq!(run_lines("const f = () => 42\nprint(f())"), "42");
    // Block bodies with explicit return.
    assert_eq!(run_lines("const g = (x) => { return x * 2 }\nprint(g(4))"), "8");
    // Curried arrows.
    assert_eq!(run_lines("const c = (a) => (b) => a + b\nprint(c(5)(3))"), "8");
    // Arrows as arguments.
    assert_eq!(
...
fn finalize_python_embed_clean_teardown_via_cli() {
    ...
    let tag = format!("fin_cli_{}_{}", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().subsec_nanos());
...
fn then_chains_and_fulfills_late() {
    assert_eq!(run_lines("Promise.resolve(5).then(v => print(v * 2))"), "10");
    assert_eq!(
        run_lines(
            "const d = Promise.withResolvers()\n\
...
fn await_outside_async_is_compile_error() {
    assert!(matches!(compile_err("print(await 5)"), CompileError::AwaitOutsideAsync));
...
fn try_catch_catches_thrown_value() {
    ...
    assert_eq!(run_lines("try { throw 7 } catch { print(\"no binding\") }"), "no binding");
...
fn labeled_blocks_and_shadowing() {
    ...
    assert!(matches!(compile_err("outer: { continue outer }"), CompileError::ContinueNonLoop(_)));
    assert!(matches!(compile_err("break nope"), CompileError::UndefinedLabel(_)));
}
...
fn switch_break_continue_labels_finally() {
    ...
    assert!(matches!(compile_err("switch (1) { case 1: continue }"), CompileError::BreakOutsideLoop));
    ...
    assert!(matches!(compile_err("outer: switch (1) { case 1: continue outer }"), CompileError::ContinueNonLoop(_)));
...
fn ternary_operator() {
    ...
    assert_eq!(run_lines("print(false ? \"a\" : true ? \"b\" : \"c\")"), "b");
    assert_eq!(run_lines("print(false ? \"a\" : false ? \"b\" : \"c\")"), "c");
    ...
    assert_eq!(run_lines("let v = 5\nprint(`v is ${v > 3 ? \"big\" : \"small\"}`)"), "v is big");
    // In loop conditions.
    assert_eq!(
...
fn increment_decrement_operators() {
    ...
    assert_eq!(run_lines("const o = { n: 1 }; let old = o.n++; print(old, o.n)"), "1 2");
    assert_eq!(run_lines("const o = { n: 1 }; let pre = ++o.n; print(pre, o.n)"), "2 2");
    assert_eq!(run_lines("const arr = [1, 2, 3]; let v = arr[1]++; print(v, arr[1])"), "2 3");
    assert_eq!(run_lines("const arr = [1, 2, 3]; let w = ++arr[0]; print(w, arr[0])"), "2 2");
    ...
    assert_eq!(run_lines("let r = 0; let res = r++ + ++r; print(r, res)"), "2 2");
...
fn compound_assign_evaluates_target_once() {
    ...
    assert_eq!(
        run_lines("const a = [10]; a[0] -= 3; print(a[0])"),
        "7"
    );
    // The expression result is the new value.
    assert_eq!(run_lines("const o = { n: 1 }; let r = o.n += 3; print(r, o.n)"), "4 4");
}
...
fn plain_assign_evaluates_target_before_rhs() {
    ...
    assert_eq!(run_lines("const o = {}; let r = o.x = 42; print(r, o.x)"), "42 42");
    assert_eq!(
    ...
    assert_eq!(
        run_lines("const a = [1]; function fill() { return [9, 8] }\na[0] = fill()[1]; print(a[0])"),
        "8"
    );
}
...
fn destructuring_assignments() {
    ...
    assert_eq!(run_lines("let s = 0; let t = 0;\n[s, t] = [7, 8]\nprint(s, t)"), "7 8");
    // Swap through destructuring.
    assert_eq!(run_lines("let a = 1; let b = 2;\n[a, b] = [b, a]\nprint(a, b)"), "2 1");
    ...
    assert_eq!(
        run_lines("let arr = [1, 2]; let f = 0; let g = 0;\nlet r = ([f, g] = arr)\nprint(r === arr)"),
        "true"
    );
...
fn spread_operator() {
    ...
    assert_eq!(run_lines("const a = [1]; const b = [2]; print([...a, ...b].length)"), "2");
    assert_eq!(run_lines("const a = [1]; print([...[...a], 2].length)"), "2");
    ...
    assert_eq!(run_lines("const s = [...\"ab\"]\nprint(s.length, s[0], s[1])"), "2 a b");
    // Into native calls.
...
fn repl_function_prints_and_mutates_globals_across_lines() {
    ...
    let line2 = Compiler::compile_source_with_mode(
        "function inc() { x = x + 1; print(x) }",
        true,
        false,
    )
    .unwrap();
...
fn rest_parameters() {
    ...
    assert_eq!(run_lines("function cat(a, b, ...rest) {\nprint(a, b, rest.length)\n}\ncat(9)"), "9 undefined 0");
    // Zero args.
    assert_eq!(run_lines("function all(...x) { print(x.length) }\nall()"), "0");
    // Arrows and closure capture of the rest array.
    assert_eq!(run_lines("let f = (...args) => args.length\nprint(f(1, 2, 3))"), "3");
    assert_eq!(run_lines("let g = (a, ...rest) => rest[0] + a\nprint(g(10, 20, 30))"), "30");
    ...
    // Rest must be last.
    assert!(matches!(compile_err("function f(...a, b) {}"), CompileError::UnexpectedToken(_)));
    assert!(matches!(compile_err("function f(a, ...b, c) {}"), CompileError::UnexpectedToken(_)));
}

#[test]
fn rest_elements_in_destructuring() {
    // Basic rest element.
    assert_eq!(run_lines("let [head, ...tail] = [1, 2, 3, 4]\nprint(head, tail.length, tail[0], tail[2])"), "1 3 2 4");
    // Holes skip elements before the rest.
    assert_eq!(run_lines("let [a, , ...rest] = [10, 20, 30, 40]\nprint(a, rest.length, rest[0])"), "10 2 30");
    ...
    assert_eq!(run_lines("let [p, [q, ...r]] = [1, [2, 3, 4]]\nprint(p, q, r.length, r[1])"), "1 2 2 4");
    assert_eq!(run_lines("let [m, ...objs] = [1, { v: 2 }, { v: 3 }]\nprint(m, objs.length, objs[1].v)"), "1 2 3");
    // Rest in the assignment form.
    assert_eq!(run_lines("let s = 0; let t = 0;\n[s, ...t] = [7, 8, 9]\nprint(s, t.length, t[0], t[1])"), "7 2 8 9");
    assert_eq!(run_lines("let r = 0;\n[...r] = [1, 2]\nprint(r.length)"), "2");
    // Strings slice by character.
    assert_eq!(run_lines("let [c1, ...cs] = \"hello\"\nprint(c1, cs.length, cs[0], cs[3])"), "h 4 e o");
...
fn nan_and_infinity_globals() {
    ...
    assert_eq!(run_lines("print(typeof NaN, typeof Infinity)"), "number number");
    // JS: NaN is falsy.
    assert_eq!(run_lines("print(NaN ? 1 : 0)"), "0");
    // NaN/Infinity cannot be shadowed.
    assert!(matches!(compile_err("let NaN = 5"), CompileError::CannotShadowBuiltin(_)));
    assert!(matches!(compile_err("let Infinity = 5"), CompileError::CannotShadowBuiltin(_)));
}
...
fn compound_assign_all_operators() {
    ...
    assert_eq!(run_lines("let arr = [2, 3]; arr[0] *= 5; arr[1] /= 2; print(arr[0], arr[1])"), "10 1.5");
...
fn array_literal_elisions() {
    ...
    assert_eq!(run_lines("print([1, , 3].length, [1, , 3][1])"), "3 undefined");
    assert_eq!(run_lines("let x = [1, , 3]; print(x[0], x[1], x[2])"), "1 undefined 3");
    // Elisions in destructuring assignments skip the element.
    assert_eq!(run_lines("let t1 = 0; let t3 = 0;\n[t1, , t3] = [9, 99, 8]\nprint(t1, t3)"), "9 8");
...
fn import_sugar_matches_node() {
    ...
    w("mid.ajs", "import { double } from './math.ajs';\n\
...
fn node_style_module_resolution() {
    ...
    w("math.ajs", "export function fromAjs() { return \"ajs\"; }\n");
    w("onlyax_src.ajs", "export function fromAx() { return \"ax\"; }\n");
    ...
    w("pkg/index.ajs", "export function fromIndex() { return \"index\"; }\n");
    w("pkg/package.json", "{\"main\": \"./entry.ajs\"}\n");
    w("pkg/entry.ajs", "export function fromMain() { return \"main\"; }\n");
    w("node_modules/mylib/package.json", "{\"main\": \"lib.js\"}\n");
    w("node_modules/mylib/lib.js", "export function fromLibMain() { return \"libmain\"; }\n");
    w("node_modules/mylib/index.ajs", "export function fromIndex() { return \"idx\"; }\n");
    w("node_modules/subpkg/index.ajs", "export function fromNodeSub() { return \"nsub\"; }\n");
    w("sub/util.ajs", "export function fromSub() { return \"sub\"; }\n");
    w("sub/mid.ajs", "const u = require('./util');\nconst sp = require('subpkg');\nexport function go() { return u.fromSub() + ' ' + sp.fromNodeSub(); }\n");
    w("sub/idx_only/only.ajs", "export function fromSub() { return \"idxonly\"; }\n");
...
fn optional_chaining() {
    ...
    assert_eq!(
        run_lines("let o = { a: { b: 42 }, n: null, z: 0 };\n\
                   print(o?.a.b);\n\
                   print(o?.x?.y);\n\
                   print(o?.n?.b);\n\
                   print(o?.z);"),
    ...
    // Index access.
    assert_eq!(
        run_lines("let o = { arr: [10, 20, 30], n: null };\n\
                   print(o?.[\"arr\"]?.[1]);\n\
                   print(o?.n?.[0]);"),
        "20\nundefined"
    );
    ...
    assert_eq!(
        run_lines("function f(x, y) { return x * 10 + y; }\n\
                   let fn = null;\n\
                   print(f?.(3, 4));\n\
                   print(fn?.(3, 4));"),
        "34\nundefined"
    ...
    assert_eq!(
        run_lines("let o = { a: { b: 42 }, m: function(x, y) { return this.a.b + x + y; } };\n\
                   print(o.m?.(1, 2));\n\
                   print(o?.m?.(1, 2));\n\
                   print(o?.m?.(1));"),
        "45\n45\nNaN"
    ...
    assert_eq!(
        run_lines("function g() { return { p: 7 }; }\n\
                   let gn = null;\n\
...
fn object_keys_entries_values() {
    ...
    assert_eq!(
        run_lines("print(Object.keys(5).length);\n\
...
fn map_set_prototype_methods_with_this() {
    ...
    assert_eq!(run_lines("let m = new Map();\nprint(Object.keys(m).length)"), "0");
...
fn arguments_object() {
    ...
    assert_eq!(
        run_lines("function snap() { return [...arguments].join(\"-\"); }\n\
                   print(snap(7, 8, 9));\n\
...
fn missing_arguments_are_undefined() {
    assert_eq!(run_lines("function f(x, y) { return x + y; }\nprint(f(1))"), "NaN");
    assert_eq!(run_lines("let o = { m: function(x, y) { return x + y; } };\nprint(o.m(1))"), "NaN");
...
fn arrow_lexical_this_and_arguments() {
    ...
    assert_eq!(
        run_lines("let o = { v: 7, f: function () { let a = () => this.v; return a(); } };\nprint(o.f())"),
        "7"
    );
...
fn modern_syntax_surface() {
    ...
    assert_eq!(run_lines("print((null ?? 0) || 5, 5 && (null ?? 9))"), "5 9");
